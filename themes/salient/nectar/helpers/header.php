			...
			if ( $network_name === 'rss' ) {
				...
				if ( ! empty( $nectar_options[ 'use-' . $network_name . '-icon-header' ] ) && $nectar_options[ 'use-' . $network_name . '-icon-header' ] === '1' ) {
					...
					if( isset($nectar_options[ $network_name . '-url' ]) ) {
						echo '<a style="color:#ffffff" '.$target_attr.' href="' . esc_url( $nectar_options[ $network_name . '-url' ] ) . '"><i class="' . esc_attr($icon_class) . '"></i> </a>';
					} else {
	...
	function nectar_header_button_items() {
	  ...
	  !empty( $nectar_options['header-slide-out-widget-area-menu-btn-bg-color'] ) ) {
		...
		if ( $user_account_btn != 'false' ) {
			...
			<a href="' . $user_account_btn_url . '">
			<img style="margin-bottom: 0;
                            width: 32px;" src="/wp-content/themes/salient/nectar/assets/img/icons/user.svg"/>
			<span style="font-size:12px;color:#E64F38">Login</span>
			</a></li>';
