...
class Advanced_Ads_Pro_Module_Advanced_Visitor_Conditions {
	...
	static function metabox_cookie( $options, $index = 0, $form_name = '' ) {
		...
		<div class="clear"></div>
		<p class="description"><?php echo $type_options[ $options['type'] ]['description']; ?> <?php _e( 'Set the operator to "match/does not match" and leave the value empty to check only the existance of the cookie.', 'advanced-ads-pro' ); ?></p><?php
	}
	...
	public static function check_cookie( $options = array() ) {
		...
		return Advanced_Ads_Visitor_Conditions::helper_check_string( $_COOKIE[ $options['cookie'] ], $options );
	}
