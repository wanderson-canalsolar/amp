        ...
        */
		...
		process_passive_cb: function() {
			...
			var fn = function( adblocker_active ) {
				...
				var waiting_for_ads_inject = setInterval(function() {
					if (ads_inject_done === self.injected) {
						self.injected = 0;
						self.busy = false;
	...
	if (typeof advads !== 'undefined' && typeof advads.privacy.dispatch_event !== 'undefined') {
		...
		document.addEventListener('advanced_ads_privacy', function (event) {
			...
			if (event.detail.state === 'accepted' || event.detail.state === 'not_needed') {
				...
				document.querySelectorAll('script[type="text/plain"][data-tcf="waiting-for-consent"]').forEach(function (node) {
