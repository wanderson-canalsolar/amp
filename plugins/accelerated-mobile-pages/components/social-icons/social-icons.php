...
function ampforwp_framework_get_social_icons($selected_social_icons){
	...
	*/
	...
	foreach ($selected_social_icons as $key => $value) {
	 ...
	 if ( isset($selected_social_icons['youtube']) && null == $selected_social_icons['youtube'] ) {
	 	$selected_social_icons['youtube'] = 'https://youtube.com/share/url?url='. $url. '&text='. $title .'';
	 	//https://youtube.com/share/url?url={url}&text={title}
	}

	if ( isset($selected_social_icons['instagram']) && null == $selected_social_icons['instagram'] ) {
		$selected_social_icons['instagram'] = 'https://instagram.com/share/url?url='. $url. '&text='. $title .'';
		//https://instagram.com/share/url?url={url}&text={title}
   }
...
.a-so li:before{box-sizing: initial;color: #C0CDD8;display: inline-block;width: 18px;height: 18px;line-height: 18px;}
