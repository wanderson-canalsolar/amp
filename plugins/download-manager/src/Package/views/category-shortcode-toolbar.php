...
if ($toolbar) {
    ...
    if((int)$title === 1) {
        ...
        foreach ($categories as $slug) {
            if(term_exists($slug, "wpdmcategory")) {
                $category = get_term_by('slug', $slug, "wpdmcategory");
                $titles[] = $category->name;
                if(trim($category->description) !== '')
                    $descs[] = $category->description;
                $icon = $icon ?: \WPDM\__\UI::img(WPDM()->categories->icon($category->term_id), "Icon", ['style' => "width: {$icon_width}px;"]);
            }
        }
        $title = implode(", ", $titles);
        $desc = implode("<hr/>", $descs);
    }
?>
